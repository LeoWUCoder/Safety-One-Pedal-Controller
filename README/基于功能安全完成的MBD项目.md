...
![image-20260820234557305](./assets/image-20260820234557305.png)

只有 `TorqueRequest_Nm` 失败：

| 用例                  | 时间         | 基线  | 当前仿真 | 最大差值 |
| --------------------- | ------------ | ----- | -------- | -------- |
| `Throttle_Pedal_Fail` | 0.32～0.39 s | 4 Nm  | 40 Nm    | 36 Nm    |
| `ATselectorCAN_Fail`  | 0.31 s       | 40 Nm | 0 Nm     | 40 Nm    |
...
因此不要修改集成Test Sequence。

问题在control_model/Chart：模型里画成了：

```
默认转移 → NoFailureDetected

默认转移 → LimpMode
条件：[isFailMode ~= 0]
动作：{lastTimeMode = AutomaticTransmissionState;}
```

也就是说，进入 `LimpMode` 的条件线现在是一条“无源默认转移”，而不是从 `NoFailureDetected` 出发的普通转移。

模型启动时没有故障，所以它进入 `NoFailureDetected`。后面故障发生时，因为缺少：
...
NoFailureDetected
    │
    │ [isFailMode ~= 0]
    │ {lastTimeMode = AutomaticTransmissionState;}
    ↓
LimpMode
```

![image-20260820234913594](./assets/image-20260820234913594.png)



![image-20260821000534845](./assets/image-20260821000534845.png)

模型和场景都没错，是测试文件时漏掉了原项目给车速信号设置的专用绝对容差。

最新 3 个失败全部来自 `Vehicle_Speed_km_h:1`：

| 测试用例           | 最大误差       | 其他三个输出 |
| ------------------ | -------------- | ------------ |
| Normal Driving     | 0.0327336 km/h | 全部通过     |
| BreakCAN_Fail      | 0.2501113 km/h | 全部通过     |
| ATselectorCAN_Fail | 0.2501113 km/h | 全部通过     |
| SpeedCAN_Fail      | 0              | 通过         |

我核对后的关键差异是：

| 设置                              | 原项目 Test.mldatx | 你的 TestFile.mldatx |
| --------------------------------- | ------------------ | -------------------- |
| 基线全局绝对容差                  | 0                  | 0                    |
| `Vehicle_Speed_km_h` 信号绝对容差 | **1**，单独覆盖    | **0，继承全局值**    |
| 相对容差                          | 0                  | 0                    |
| 时间容差                          | 0                  | 0                    |
| 车速插值方式                      | linear             | linear               |

也就是说，并不是用零容差比较车速，而是允许车速输出与基线相差最多 `1 km/h`。你复现时把这个信号级设置漏掉了。
...
基线比较使用 `union + linear`，因此会在 `1.004、4.8755、5.3083、8.1126、9.9447` 等非固定步长时间点进行插值。再叠加车速使用 `single` 类型，就会出现少量插值和浮点误差。

例如：

| 时间   | 基线    | 仿真比较值 | 误差     |
| ------ | ------- | ---------- | -------- |
| 1.004  | 13.6940 | 13.6688    | -0.02515 |
| 4.8755 | 31.0980 | 31.1307    | +0.03273 |
| 8.1126 | 31.4720 | 31.4421    | -0.02990 |
| 9.9447 | 5.9090  | 5.6589     | -0.25011 |

这些误差都小于项目规定的 `1 km/h`。

![image-20260821001353031](./assets/image-20260821001353031.png)

![image-20260821001620718](./assets/image-20260821001620718.png)

拿下：

![image-20260821001749361](./assets/image-20260821001749361.png)
